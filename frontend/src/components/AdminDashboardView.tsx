            ...
            <div className="space-y-2">
              {ingredients.map((ing, i) => (
                <div key={i} className="flex items-center gap-2">
                  <input className="form-input flex-1" placeholder="食材名称" value={ing.customName} onChange={(e) => updateIngredient(i, "customName", e.target.value)} />
                  <input className="form-input w-20" placeholder="用量" value={ing.amount} onChange={(e) => updateIngredient(i, "amount", e.target.value)} />
                  <input className="form-input w-16" placeholder="单位" value={ing.unit} onChange={(e) => updateIngredient(i, "unit", e.target.value)} />
                  <select className="form-input w-20" value={ing.groupName} onChange={(e) => updateIngredient(i, "groupName", e.target.value)}>
                    <option value="主料">主料</option><option value="腌料">腌料</option><option value="配料">配料</option><option value="调料">调料</option>
                  </select>
                  <select className="form-input w-24" value={ing.scaleType} onChange={(e) => updateIngredient(i, "scaleType", e.target.value)}>
                    <option value="linear">等比</option><option value="sub_linear">亚线性</option><option value="fixed">固定</option>
                  </select>
                  {ingredients.length > 1 && <button onClick={() => removeIngredient(i)} className="text-gray-300 hover:text-red-500"><X className="w-4 h-4" /></button>}
                </div>
              ))}
            </div>
          </div>
